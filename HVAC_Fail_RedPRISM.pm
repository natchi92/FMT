...
const int DEG_N7 = 3;
const int DEG_N8 = 5;
const double DEG_MTTF1 = 3426.3787;
const double DEG_MTTF2 = 3422.6748;
...
module EBE1
	...
	[trigger] s1 = 0 -> (s1'=1);
    // Degradation
	[  ] 0 < s1 & s1 < DEG_N1-1 -> (DEG_N1-1)/ DEG_MTTF1 : (s1'=s1+1);
	[f1]          s1 = DEG_N1-1 -> (DEG_N1-1)/ DEG_MTTF1 : (s1'=s1+1);
	// Maintenance: clean
	[cln] s1 > 1 -> (s1'=s1-1);
	[cln] s1 = 1 -> true;
    // Maintenance: replace
...
endrewards
//
rewards "cost_rplc"  // the triggered action
	[rplc] true : COST_RPLC;
endrewards
rewards "cost_cln"
	[cln] true : COST_CLN;
